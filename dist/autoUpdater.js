...
class ElectronGithubAutoUpdater extends EventEmitter {
    ...
    _registerInterceptors = () => {
        if (this.shouldForwardEvents) {
            ...
            electronAutoUpdater.on('update-available', () => this.emit('update-downloaded', {
    ...
    getLatestRelease = async () => {
        const response = await axios.get(`${this.baseUrl}/repos/${this.owner}/${this.repo}/releases?per_page=100`, {
        ...
        if (this.allowPrerelease) {
            ...
            const matchedRelease = releases.find((release) => !release.prerelease);
            if (!matchedRelease)
    ...
    _loadElectronAutoUpdater = (release) => {
        if (!isDev) {
    ...
    downloadUpdateFromRelease = async (release) => {
        try {
    ...
    checkForUpdates = async (releaseType = 'default') => {
        try {
